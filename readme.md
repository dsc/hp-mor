...
Instead of attempting to identify each dialogue line, the script looks for a character's name preceeded or followed by a "said" word. I first extracted a [giant] list of all the words that either preceeded or followed the name of a main character, and aggregated it by frequency. (It looks [like this](counts/words/totals.txt), and there are also [chapter counts](counts/words/), if you care.) I went through that list by hand to pull out words that were synonyms (for our purposes) of "said" in a dialogue line's speaking indicator, like "Harry said". (Here's [the list of words](etc/said-words.txt), and [one with counts](etc/said-counts.yaml).)

Then a second script went through everything again, this time only counting instances of those words preceeding or following a main character name, and keeping count by character. The totals are what you see above; the by-chapter tallys follow.

You can download a tar of all this on github.
